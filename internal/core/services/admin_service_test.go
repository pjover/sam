...
func Test_Backup_ok(t *testing.T) {
	...
	mockedExecManager := new(os_mocks.ExecManager)
	mockedExecManager.On("Run", "mongoexport", "--db=hobbit", "--collection=consumption", "--out=consumption.json").Return(nil)
	mockedExecManager.On("Run", "mongoexport", "--db=hobbit", "--collection=customer", "--out=customer.json").Return(nil)
	mockedExecManager.On("Run", "mongoexport", "--db=hobbit", "--collection=invoice", "--out=invoice.json").Return(nil)
	mockedExecManager.On("Run", "mongoexport", "--db=hobbit", "--collection=product", "--out=product.json").Return(nil)
	mockedExecManager.On("Run", "mongoexport", "--db=hobbit", "--collection=sequence", "--out=sequence.json").Return(nil)
