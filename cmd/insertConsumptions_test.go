...
func Test_ParseInsertConsumptionsArgs(t *testing.T) {
	...
	t.Run("Should parse with 1 consumption", func(t *testing.T) {
		...
		assert.Equal(t, core.InsertConsumptionsArgs{Code: 1552, Consumptions: map[string]float64{"QME": 1}, Note: testNote}, actual)
		assert.Nil(t, err)
	})

	t.Run("Should parse with 2 consumption", func(t *testing.T) {
		var actual, err = parseInsertConsumptionsArgs([]string{"2220", "0.5", "MME", "2", "AGE"}, testNote)
		assert.Equal(t, core.InsertConsumptionsArgs{Code: 2220, Consumptions: map[string]float64{"MME": 0.5, "AGE": 2}, Note: testNote}, actual)
		assert.Nil(t, err)
	})
	...
	t.Run("Should parse with 3 consumption", func(t *testing.T) {
		...
		assert.Equal(t, core.InsertConsumptionsArgs{Code: 2220, Consumptions: map[string]float64{"MME": 0.5, "AGE": 2, "QME": 1, "BAB": 1}, Note: testNote}, actual)
		assert.Nil(t, err)
	})
}
